...
from numpy import zeros, isnan, min, max
from .fuzzification import *

class FuzzyTreeMod(object):
...
#            raise Exception("Invalidy type for the Fuzzy Set")
    ...
    def _createTree(self):
        ...
        #.....................................................................
        #STEP 1: Measure the classification similarity associated
        #        with each attribute
        
        # Computes Similarity between class and all of the subsets of a fuzzification features
        
        mFvar = "" # Name of the FuzzyVar with more similarity
        Pool = {}
        for P in self._LHS:
            try:
                FSamb = ClassSimilarity(fs_C, theFuzzySet[P],P)
            except:
                "error"

            Pool.update(FSamb)

        #Choose the max fuzzification tree to start building the pattern tree
        Pool = {k: v for k, v in sorted(Pool.items(), key=lambda item: item[1],reverse=True)}
        mFvar = list(Pool.keys())[0]
        maxi = Pool[mFvar]
           
        # Need to figure out how to save the combinations No parent nodes as fuzzified feature ling will be leafs. Inner nodes are combinations
        
                
        self._NodeParent  = FuzzyTreeNode(FVarName = mFvar)
        self._NodeParent.Truth = maxi
        ...
        #iterate through simple trees
        for slave in Pool:
            #define aggregation functions
            aggList = {"AND":min, "OR":max}

            for agg in aggList:
                bestAgg = None
                if bestAgg == None:
                    bestSet = aggregate(candidate,slave,agg)
                    bestAgg = agg
                else:
                    if bestSet < aggregate(candidate,slave,agg):
                        bestSet = aggregate(candidate,slave,agg)
                        bestAgg = agg
